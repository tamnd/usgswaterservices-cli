...
describes what users get. Pick a format with `-o`, or let usgswater choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
usgswater <command> -o table   # aligned columns for reading
usgswater <command> -o jsonl   # one JSON object per line, for piping
usgswater <command> -o json    # a single JSON array
usgswater <command> -o csv     # spreadsheet friendly
usgswater <command> -o tsv     # tab-separated
usgswater <command> -o url     # just the URL column
usgswater <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
usgswater <command> --fields id,title,url
```
...
```bash
usgswater <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
usgswater <command>            # a table, because this is a terminal
usgswater <command> | wc -l    # JSONL, because this is a pipe
```
